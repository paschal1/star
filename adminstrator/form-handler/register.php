...
if (isset($_POST['register'])){
    ...
    $sql = mysqli_query($mysqli, "SELECT * FROM admins WHERE email = '$email'");
    ...
    if(empty($name) && (empty($email) && (empty($password) && (empty($cpassword))))){
     ...
     if(empty($error)){
        ...
        $role = "Admin";
        $query = mysqli_query($mysqli, "INSERT INTO admins VALUES('','$name','$email','$hashedpwd','$chashedpwd','$profile_pic','$role','0');");
        if($query){
